...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $verified =  0;
    $payment_status = isset($_POST['payment_status']) ? $_POST['payment_status'] : 0;
    ...
    if (!empty($user_name) && !empty($user_email) && !empty($user_password) && !empty($user_type)) {
        ...
        if ($checkEmail->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO user_profile (
                ...
                user_preference, vkey, verified, payment_status, date_added
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            $stmt->bind_param("sssssssssssdsiiidssiss",
